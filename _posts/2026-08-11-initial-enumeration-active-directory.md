...
When you send a Kerberos AS-REQ for a valid user, the DC responds with a specific error (`KDC_ERR_PREAUTH_REQUIRED`) - basically saying "I know this user, give me your pre-authentication." For an invalid user, the DC responds with a completely different error (`KDC_ERR_C_PRINCIPAL_UNKNOWN`) - "never heard of them."
...
Here's the concept: Active Directory uses **Service Principal Names (SPNs)** to associate services with accounts. An SPN is basically a unique identifier that says "this account runs this service." Any authenticated user in the domain can query the DC for a list of all accounts that have SPNs registered.
