...
This post fills in that gap. We'll cover how Kerberos actually works, why it's different from NTLM, and then break down exactly what AS-REP Roasting and Kerberoasting each exploit - and why they're not the same attack even though they're often mentioned in the same breath.
...
If AS-REP Roasting didn't pan out - or even if it did and we already have a set of credentials - we can go after Kerberoasting instead. The logic here is simple once you've seen the AS-REQ/AS-REP flow.
...
This is why Kerberoasting isn't really a misconfiguration in the way AS-REP Roasting is. Pre-auth being disabled is a mistake someone made. Service accounts having SPNs and Kerberos allowing anyone to request tickets for them - that's just how the protocol is designed to work. The primary mitigation is making sure service account credentials are strong enough to resist offline cracking - ideally long, random passwords or gMSAs with automatic rotation, and avoiding legacy RC4 encryption where possible.
